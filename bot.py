...
async def init_products(context):
    try:
        with LOCK:
            conn = sqlite3.connect(DB_FILE, check_same_thread=False, timeout=15)
            try:
...
async def check_pending_invoices(context):
    try:
        with LOCK:
            conn = sqlite3.connect(DB_FILE, check_same_thread=False, timeout=15)
            try:
...
async def scheduled_tasks(app):
    ...
    await init_products(None)
    while True:
        await asyncio.sleep(60)
        await check_pending_invoices(None)
...
if __name__ == '__main__':
    ...
    asyncio.get_event_loop().create_task(scheduled_tasks(application))
